...
int main(int argc, char ** argv)
{
  ...
  const std::string keys =
    ...
    "{@input | ../../data/vtest.avi | Input image or video}";
  ...
  // readNetFromONNX THROWS a cv::Exception if the file is missing or
  // unreadable (it does not return an empty net), so the friendly hint
  // about export_model.py must live in a catch block
  cv::dnn::Net net;
  try {
    net = cv::dnn::readNetFromONNX("../../data/models/yolo11/yolo11n.onnx");
  } catch (const cv::Exception &) {
    // Two very different causes end up here, so tell them apart: if the file
    // is on disk the parser is the problem, and with YOLO11 that almost
    // always means an OpenCV older than 4.9 (its ONNX importer cannot handle
    // the Split node of the attention block)
    const bool exists = std::ifstream("../../data/models/yolo11/yolo11n.onnx").good();
    std::cerr << "Error: cannot load ../../data/models/yolo11/yolo11n.onnx" << std::endl;
    if (exists) {
      std::cerr << "The file is there, so this is almost certainly the OpenCV "
                << "version: YOLO11 needs >= 4.9 and you have "
                << cv::getVersionString() << std::endl;
    } else {
      std::cerr << "Generate it first with: python3 export_model.py" << std::endl;
    }
    return EXIT_FAILURE;
  }
  ...
  while (true) {
    ...
    // Note: blobFromImage below STRETCHES the frame to 640x640 (no
    // letterboxing), so x and y need independent scale factors. Ultralytics'
    // own Python inference letterboxes instead (pads to keep aspect ratio),
    // which is why its raw confidences can differ slightly from ours.
