...
int main(int argc, char ** argv)
{
  ...
  std::string filename = cv::samples::findFile(parser.get<std::string>("@input"), false);
  if (!parser.check()) {
    parser.printErrors();
    return EXIT_FAILURE;
  }
  ...
  while (true) {
    ...
    cv::calcOpticalFlowPyrLK(
      ...
      criteria          // Stop after 10 iterations or when the point position
                        // shifts less than 0.03 px between iterations (EPS)
    );
    ...
    //
    // Practical limitation of Lucas-Kanade: points lost to occlusion or
    // large motion are dropped and NEVER replaced, so the tracked set only
    // shrinks over time. Real applications re-run goodFeaturesToTrack()
    // periodically (e.g. when the point count falls below a minimum) to
    // replenish the pool -- try adding that here as an exercise.
