 ...
 * - Computing a disparity map with StereoSGBM (same idea as 11_02)
 ...
 * follow: the equation presented in 11_02,
 *
 *     Z = f * b / disparity
 *
 ...
 * calibration (Chapter 10). This example accepts that file directly:
 *
 *     ./11_03_stereo_to_pointcloud left.png right.png --calib=stereo_calibration.yml
 *
 * where stereo_calibration.yml is what 10_03_stereo_calibration writes. That
...
{
  // Load the rectified stereo pair (same images as 11_02)
  ...
  // during stereo calibration, which is exactly what 10_03_stereo_calibration
  ...
  // 10 and the 3D of chapter 11, and it does two things, not one: it rectifies
  ...
  if (!calib_file.empty()) {
    ...
    if (Q.empty() || Q.rows != 4 || Q.cols != 4) {
      std::cerr << "Error: " << calib_file << " has no valid 4x4 Q matrix. "
                << "Generate it by running 10_03_stereo_calibration." << std::endl;
      return EXIT_FAILURE;
    }
    ...
    // Rectification: the same remap that 10_03 applies, so that corresponding
    ...
    std::cout << "No --calib given: using an assumed rig (focal "
              ...
              << "10_03_stereo_calibration and pass its\n  stereo_calibration.yml "
  ...
  // ========================================
  // Minimal SGBM setup (11_02 explores the parameters and post-filtering
  ...
  // SGBM returns fixed-point disparities scaled by 16 (see 11_02):
