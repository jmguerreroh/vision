...
int main(int argc, char ** argv)
{
  ...
  // it uses PCL's own argument parser. The example builds its own data,
  // so it takes no input file
  ...
  if (icp.hasConverged()) {
    ...
    // Compare with the known transformation.
    //
    // ICP convention (easy to get wrong!): with setInputSource(source) and
    // setInputTarget(target), align() estimates the transformation T such
    // that  T * source = target. Since we built the target as
    // target = known_transform * source, the estimate must satisfy
    // T_est = known_transform -- NOT its inverse. (You would compare against
    // the inverse only if the source and target roles were swapped.)
    std::cout << "\n=== Comparing with Known Transformation ===" << std::endl;
    std::cout << "Expected transformation (the one we applied):" << std::endl;
    printTransformationMatrix(known_transform);

    std::cout << "\nTransformation Error (estimated - expected, ~0 if converged):"
              << std::endl;
    Eigen::Matrix4f error = icp.getFinalTransformation() - known_transform;
