...
int main(int argc, char ** argv)
{
  ...
  //
  // IMPORTANT: imshow() always interprets a 3-channel image as BGR.
  // When we display 'hsv' directly we are NOT seeing "HSV colors": the H,
  // S and V values are being reinterpreted as blue, green and red. That is
  // why these windows look strange -- and why the per-channel visualization
  // below is the meaningful way to inspect a converted color space.
  ...
  cv::imshow("HSV - Hue", hsv_channels[0]);
  cv::imshow("HSV - Saturation", hsv_channels[1]);
  cv::imshow("HSV - Value", hsv_channels[2]);
  ...
  cv::imshow("BGR (converted back from HSV)", bgr_from_hsv);
