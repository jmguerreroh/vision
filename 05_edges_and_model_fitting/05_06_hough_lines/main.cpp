...
int main(int argc, char ** argv)
{
  ...
  for (size_t i = 0; i < lines.size(); i++) {
    ...
    // (cos t, sin t) is the line's NORMAL direction, so (-sin t, cos t) is
    // its DIRECTION vector. Walking +-1000 px along it from (x0, y0) gives
    // two endpoints guaranteed to be outside the image, so the drawn
    // segment always crosses the full view
