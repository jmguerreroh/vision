...
int main(int argc, char ** argv)
{
  ...
  const cv::Mat img_base = cv::imread(
    cv::samples::findFile(base_path + "Histogram_Comparison_Source_0.jpg"));
  const cv::Mat img_test1 = cv::imread(
    cv::samples::findFile(base_path + "Histogram_Comparison_Source_1.jpg"));
  const cv::Mat img_test2 = cv::imread(
    cv::samples::findFile(base_path + "Histogram_Comparison_Source_2.jpg"));
  ...
  for (int method = 0; method < 4; method++) {
    const std::vector<double> scores = compareWithReferences(hist_base, references, method);
