...
int main () {
    ...
    putText(bw_otsu, to_string(otsu_val), Point(bw_otsu.cols-100, 15), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255));

    // Convert image to binary using Otsu
    double bin_val = cv::threshold(gray, bw_bin, 100, 255, THRESH_BINARY);
    putText(bw_bin, to_string(bin_val), Point(bw_bin.cols-100, 15), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255));
    ...
    putText(bw_otsu_bin, to_string(otsu_comb_val), Point(bw_otsu_bin.cols-100, 15), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255));
