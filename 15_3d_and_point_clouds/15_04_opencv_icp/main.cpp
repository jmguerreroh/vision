 ...
 * For PCL-based ICP, see 15_09_pcl_icp.
 *
 * Usage: ./15_04_opencv_icp <source.ply> <target.ply>
 *
...
int main(int argc, char ** argv)
{
  ...
  cv::CommandLineParser parser(argc, argv,
    ...
    "{@source | ../../data/parasaurolophus_source.ply | Source point cloud}"
    "{@target | ../../data/parasaurolophus_target.ply | Target point cloud}");
  if (parser.has("help")) {
    parser.printMessage();
    return EXIT_SUCCESS;
  }
  ...
  // Save aligned point cloud to the current directory (the data/ folder is
  // treated as read-only source material for the book)
  std::string output_file = "parasaurolophus_aligned_output.ply";
