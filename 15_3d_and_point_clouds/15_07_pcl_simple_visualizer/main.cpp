 ...
 *
 * Usage: ./15_07_pcl_simple_visualizer [path_to_pcd_file]
...
void displayCloudStatistics(const pcl::PointCloud<pcl::PointXYZ>::Ptr & cloud)
{
  ...
  // Bounding-box center vs centroid: the bbox center depends only on the
  // extreme points; the centroid is the MEAN of all points and represents
  // the actual mass distribution of the cloud. They differ whenever the
  // points are not symmetrically distributed.
  float bx = (min_pt.x + max_pt.x) / 2.0f;
  float by = (min_pt.y + max_pt.y) / 2.0f;
  float bz = (min_pt.z + max_pt.z) / 2.0f;
  std::cout << "\nBounding-box center: (" << bx << ", " << by << ", " << bz << ")" << std::endl;

  Eigen::Vector4f centroid;
  pcl::compute3DCentroid(*cloud, centroid);
  std::cout << "Centroid (mean of all points): (" << centroid[0] << ", "
...
int main(int argc, char ** argv)
{
  ...
  std::cout << "Point cloud file: " << filepath << std::endl;
