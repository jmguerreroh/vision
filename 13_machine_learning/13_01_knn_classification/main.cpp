...
static void runKNN(int K)
{
  ...
  //
  // cv::ml::KNearest performs a BRUTE_FORCE search by default: each
  // prediction compares the query against every stored sample. Fine for
  // hundreds of points; for large datasets a spatial index (KD-tree)
  // or approximate search would be needed.
  // ------------------------------------------------------------------
