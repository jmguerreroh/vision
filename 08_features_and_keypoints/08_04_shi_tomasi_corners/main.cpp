 ...
 *       a variant of Harris that ranks corners by the smaller eigenvalue of
 *       the structure tensor, selecting the most trackable features.
 *
 ...
 *
 * Algorithm Steps:
 *   1. Compute image gradients (Ix, Iy) with Sobel derivatives
...
void detectAndDrawCorners()
{
  ...
  // ========================================
  // Display Statistics (presentation only)
  //
  // Everything below is window dressing -- text boxes and backgrounds.
  // The computer-vision lesson of this example ended at
  // goodFeaturesToTrack() + the drawing loop above.
  // ========================================
...
void showHelp()
{
  ...
  std::cout << "  1. Compute image derivatives (Ix, Iy)" << std::endl;
...
int main(int argc, char ** argv)
{
  ...
  cv::CommandLineParser parser(argc, argv,
    ...
    "{@input | ../../data/checkerboard.png | Input file}");
  if (parser.has("help")) {
    parser.printMessage();
    return EXIT_SUCCESS;
  }
