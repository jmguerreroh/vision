...
int main(int argc, char ** argv)
{
  ...
  // cv::RNG with a fixed seed: same colors on every run (reproducible
  // figures for the book, unlike rand() without srand())
  cv::RNG rng(12345);
  for (size_t i = 0; i < filtered_contours.size(); ++i) {
  ...
  for (size_t i = 0; i < filtered_contours.size(); ++i) {
    ...
    cv::putText(image_with_centroids, coord_text,
                ...
                cv::FONT_HERSHEY_SIMPLEX, fontFor(image_with_centroids, Config::TEXT_FONT_SCALE),
