...
#include <opencv2/ximgproc.hpp>  // thinning() - requires opencv_contrib
#include <iostream>
#include <vector>
#include <chrono>
...
int main(int argc, char ** argv)
{
  ...
  cv::CommandLineParser parser(argc, argv,
    ...
    "{@input | ../../data/star.jpg | Input file}");
  if (parser.has("help")) {
    parser.printMessage();
    return EXIT_SUCCESS;
  }
  const std::string filename = parser.get<std::string>("@input");
  cv::Mat original = cv::imread(cv::samples::findFile(filename, false), cv::IMREAD_GRAYSCALE);
