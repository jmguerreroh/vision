 ...
 *
 *          Relation to 06_02_connected_components: floodFill labels ONE
 *          region interactively from a seed; connectedComponents labels ALL
 *          regions automatically in a single pass.
 *
 *          Fill modes:
...
static void onMouse(int event, int x, int y, int /*flags*/, void * /*userdata*/)
{
  ...
  if (app.use_mask) {
    // Keep previously filled regions visible at gray (128) while the
    // region filled by THIS click is written at mask_value (255). Without
    // this re-threshold, floodFill would refuse to cross old fills.
...
int main(int argc, char ** argv)
{
  ...
  cv::CommandLineParser parser(argc, argv,
    ...
    "{@input | ../../data/fruits.jpg | Input file}");
