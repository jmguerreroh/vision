 ...
 *
 *     ./14_03_stereo_to_pointcloud left.png right.png --calib=stereo_calibration.yml
 *
 * where stereo_calibration.yml is what 13_03_stereo_calibration writes. That
...
int main(int argc, char ** argv)
{
  ...
  // during stereo calibration, which is exactly what 13_03_stereo_calibration
  ...
  if (!calib_file.empty()) {
    ...
    if (Q.empty() || Q.rows != 4 || Q.cols != 4) {
      std::cerr << "Error: " << calib_file << " has no valid 4x4 Q matrix. "
                << "Generate it by running 13_03_stereo_calibration." << std::endl;
      return EXIT_FAILURE;
    }
    ...
    std::cout << "No --calib given: using an assumed rig (focal "
              ...
              << "13_03_stereo_calibration and pass its\n  stereo_calibration.yml "
