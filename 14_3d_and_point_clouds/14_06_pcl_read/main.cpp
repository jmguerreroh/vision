 ...
 *
 * Usage: ./14_06_pcl_read [path_to_pcd_file]
 ...
 *        14_05_pcl_write first. Running 14_05 rewrites it byte for byte,
...
int main(int argc, char ** argv)
{
  ...
  // Ptr is a shared pointer (std::shared_ptr in modern PCL) providing
  // automatic memory management; most PCL APIs take clouds by Ptr
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);

  // Load PCD file
  // loadPCDFile() returns -1 on error, 0 on success
  // Other options: loadPCDFile() with Eigen::Vector4f for sensor origin
  const std::string input_file = argc > 1 ? argv[1] : "../../data/pcl_data/test_pcd.pcd";
