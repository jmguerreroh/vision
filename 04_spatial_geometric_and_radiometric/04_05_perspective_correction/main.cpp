 ...
 *         | h21  h22  ty |
 *         | vx   vy   1  |
 *       The last row is what the affine transform of 04_04 does not have: it
...
// once with cv::findChessboardCorners (see 08_01_camera_calibration) and are
...
cv::Mat correctPerspective(const cv::Mat & src, const std::vector<cv::Point2f> & corners)
{
  ...
  // findHomography with RANSAC, used in 06_09_ransac_matching
