...
int main(int argc, char** argv)
{
    ...
    /*
    imshow( "Equalized Image B Channel", b_eqhist );
    imshow( "Equalized Image G Channel", g_eqhist );
    imshow( "Equalized Image R Channel", r_eqhist );
    */
    ...
    for( int i = 1; i < histSize; i++ )
    {
        line( histImageEq, Point( bin_w*(i-1), hist_h - cvRound(b_histeq.at<float>(i-1)) ),
              Point( bin_w*(i), hist_h - cvRound(b_histeq.at<float>(i)) ),
              Scalar( 255, 0, 0), 2, 8, 0  );
        line( histImageEq, Point( bin_w*(i-1), hist_h - cvRound(g_histeq.at<float>(i-1)) ),
              Point( bin_w*(i), hist_h - cvRound(g_histeq.at<float>(i)) ),
              Scalar( 0, 255, 0), 2, 8, 0  );
        line( histImageEq, Point( bin_w*(i-1), hist_h - cvRound(r_histeq.at<float>(i-1)) ),
