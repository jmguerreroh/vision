...
int main(int argc, char **argv) {
    ...
    for (auto const &f : fileNames) {
        ...
        // Display
        resize(img, img, Size(img.cols/2, img.rows/2));
        putText(img, "Original", Point(img.cols-100, 15), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 0), 2);
        resize(imgUndistorted, imgUndistorted, Size(imgUndistorted.cols/2, imgUndistorted.rows/2));
        putText(imgUndistorted, "Undistorted", Point(imgUndistorted.cols-100, 15), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 0), 2);
