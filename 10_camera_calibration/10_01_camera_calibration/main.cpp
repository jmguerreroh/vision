...
int main(int argc, char ** argv)
{
    ...
    if (pattern_found) {
      ...
      cv::cornerSubPix(
        ...
        cv::TermCriteria(
          ...
          0.001           // Accuracy threshold for stopping (sub-pixel)
      ));
  ...
  // Flags for calibration. Each flag REMOVES one unknown from the
  // optimization; with few or low-quality views, fixing parameters that
  // are close to their ideal value makes the estimate more stable:
  int flags = cv::CALIB_FIX_ASPECT_RATIO |    // fx = fy (square pixels assumed)
    cv::CALIB_FIX_K3 |                        // k3 only matters for wide-angle lenses
    cv::CALIB_ZERO_TANGENT_DIST |             // modern lenses are well centered
    cv::CALIB_FIX_PRINCIPAL_POINT;            // assume optical center = image center
  ...
  // The return value is the RMS reprojection error in pixels: the average
  // distance between the detected corners and the same corners re-projected
  // with the estimated parameters. Below ~0.5 px is an excellent calibration.
  ...
  // ========================================
  // The value returned by calibrateCamera is a GLOBAL average and, like any
  // average, it hides what happens in the extreme cases. Recomputing the
  // error image by image is the most useful check of the whole process: it
  // tells WHICH captures are dragging the calibration down. The usual causes
  // are visible in the image itself -- motion blur, an out-of-focus board, a
  // reflection over part of the pattern, or an angle so extreme that the far
  // corners are detected badly.
  auto perViewErrors = [&](const cv::Matx33f & Km, const cv::Vec<float, 5> & Dm,
                           const std::vector<cv::Mat> & rv,
                           const std::vector<cv::Mat> & tv) {
      std::vector<double> errors(chess_board_3d.size());
      for (std::size_t i = 0; i < chess_board_3d.size(); i++) {
        std::vector<cv::Point2f> projected;
        cv::projectPoints(chess_board_3d[i], rv[i], tv[i], Km, Dm, projected);
        errors[i] = cv::norm(chess_board_2d[i], projected, cv::NORM_L2) /
          std::sqrt(static_cast<double>(projected.size()));
      }
      return errors;
    };

  const std::vector<double> view_errors = perViewErrors(K, dist_coeffs, rvecs, tvecs);

  // Views above this error are considered unusable. It is not a magic
  // number: it simply sits well above the error of a good view (~0.3 px)
  // and well below that of a spoiled one (several pixels).
  constexpr double ERROR_THRESHOLD = 1.5;

  std::cout << "\nReprojection error per view:" << std::endl;
  std::vector<std::vector<cv::Point3f>> good_3d;
  std::vector<std::vector<cv::Point2f>> good_2d;
  for (std::size_t i = 0; i < view_errors.size(); i++) {
    const bool good = view_errors[i] <= ERROR_THRESHOLD;
    std::cout << "  [" << (i + 1) << "] " << view_errors[i] << " px"
              << (good ? "" : "   <-- discarded") << std::endl;
    if (good) {
      good_3d.push_back(chess_board_3d[i]);
      good_2d.push_back(chess_board_2d[i]);
    }
  }

  // ========================================
  // Calibrate again without the bad views
  // ========================================
  // When a calibration gives a high error, the reflex is to capture MORE
  // images. It is usually more effective to do the opposite: look at the
  // error per view and REMOVE the ones that stand out.
  const std::size_t discarded = view_errors.size() - good_3d.size();
  if (discarded > 0 && good_3d.size() >= 3) {
    cv::Matx33f K_clean(cv::Matx33f::eye());
    cv::Vec<float, 5> dist_clean(0, 0, 0, 0, 0);
    std::vector<cv::Mat> rvecs_clean, tvecs_clean;
    const double error_clean = cv::calibrateCamera(
      good_3d, good_2d, frame_size, K_clean, dist_clean,
      rvecs_clean, tvecs_clean, flags);

    std::cout << "\nDiscarding " << discarded << " of " << view_errors.size()
              << " views above " << ERROR_THRESHOLD << " px:" << std::endl;
    std::cout << "  reprojection error " << error << " -> " << error_clean
              << " px" << std::endl;
    std::cout << "  fx " << K(0, 0) << " -> " << K_clean(0, 0) << std::endl;

    // Keep the better calibration for everything that follows
    K = K_clean;
    dist_coeffs = dist_clean;
    error = error_clean;
  }
  ...
  // pose estimation of 12_02 needs exactly this file. cv::FileStorage
  ...
  // (initUndistortRectifyMap + remap) on EVERY call, which is wasteful
  // when the calibration does not change between frames.
