...
int main(int argc, char ** argv)
{
  ...
  const cv::Mat im_reference = cv::imread(cv::samples::findFile(ref_filename), cv::IMREAD_COLOR);
  ...
  const cv::Mat im = cv::imread(cv::samples::findFile(img_filename), cv::IMREAD_COLOR);
  if (im.empty()) {
